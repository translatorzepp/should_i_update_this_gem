...
RSpec.describe GemFinder do
  ...
  it "only outputs the changlog up to the next change after the version you're on" do
    full_changelog = File.read("CHANGELOG.rdoc")
    changelog_after_version = File.read("CHANGELOG_AFTER_2.47.rdoc")
    expect(subject.changelog_trimmer(full_changelog)).to eq(changelog_after_version)
  end
end
